...
项目列表默认显示活跃项目；已结束项目进入“已归档”，不会自动删除，也不会把任务改成完成。
归档前检查未结束的 Agent、审批与验收；普通未完成任务确认后可随项目暂停。
归档后保留历史读取与必要的权限撤销，停止业务写入、派单和普通催办；恢复只开启后续工作，不补跑积压自动化。

实现边界、修改原因及本地验收见 [项目归档与列表收纳](docs/项目归档与列表收纳-2026-09-23.md)。
